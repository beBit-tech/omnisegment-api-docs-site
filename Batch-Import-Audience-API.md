...
|custom_fields_identifier_field|object||自訂義欄位的識別key，適用於多個欄位的自訂義欄位用來更新指定子欄位值。 <br> for example `{"root_key_1": "sub_field_key1", "root_key_2": "sub_field_key2"}`<br> 會根據`root_key_1` 內的 子欄位 `sub_field_key1`相同的去更新對應的資料。||
