...
| coupon | string | 優惠名稱<br><br>範例: `coupon-ABC` | | - 最後來源 |
| source | string | 來源名稱<br><br>範例: `Facebook` | | |
...
| 自定義欄位 | objects | 組織需另外開啟自定義欄位功能，需依照當初設定的資料型別輸入，支援的資料型別有：datetime（日期時間）, date(日期), string(文字), 數值（Decimal)。 `{"purchase_custom_text": "text", "purchase_custom_number": 1}` <br/><br/> 請注意，訂單自定義欄位的「文字格式」最多為 256 字元。| | |
