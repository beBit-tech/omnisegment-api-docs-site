...
| file_type| File type. | **`"file_type": "Batch PII"`** | string | &#10004; | `BatchPII`, `BatchAudience`, `BatchProduct`, `TargetAudience` |
| status| The file upload status. | **`"status": "Completed"`** | string | &#10004; | `Completed`, `Running` |
| upload_id | The ID to identify which workflow's ta. | **`"upload_id": "cc411a94-2375-45fa-9b13-bc50394e791a"`** | string | | This parameter is required when file_type is `BatchPII`. You can get the ID from the [api](https://github.com/beBit-tech/omnisegment-api-docs/wiki/Provide-personal-Info-temporarily). |
| node_id | The target node ID to trigger workflow. | **`"node_id": 999"`** | integer | | This parameter is required when file_type is `TargetAudience`. |
| remove_absent | Allow to delete data. | **`"remove_absent": false`** | bool | | If just want to update audience info set the param to false. This parameter is only for file_type with `BatchAudiences` and `BatchProduct`. |
